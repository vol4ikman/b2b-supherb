...
jQuery(document).on(
	"change",
	'.sidebar-widget-item input[type="checkbox"]',
	function (e) {
		e.preventDefault();
...
jQuery("select.category-order-by").on("change", function () {
	jQuery('input[name="category-order-by"]').val(
		jQuery("select.category-order-by").val()
	);
...
jQuery(document).on(
	"click",
	".reg-step-1.active button.next-step",
	function (e) {
		e.preventDefault();
		var errors = [];
		var inputs = jQuery(
			".reg-step-1.active input, .reg-step-1.active select"
		);
...
jQuery(document).on(
	"click",
	'.reg-step-2.active button[type="submit"]',
	function (e) {
		e.preventDefault();
		var errors = [];
...
jQuery(document).on("submit", ".product-block-item form.cart", function (e) {
	e.preventDefault();

	jQuery.blockUI({
		overlayCSS: { backgroundColor: "#A1D45D", cursor: "wait" },
		message: "מוסיף מוצר לעגלה",
		css: {
			padding: 20,
			margin: 0,
			width: "30%",
	...
	var data = {
		action: "b2b_woocommerce_ajax_add_to_cart",
		form: form_data,
	};
	...
	jQuery.ajax({
		...
		success: function (response) {
			if (!response.error) {
				if (response.item_id) {
					jQuery(
						'.user-products-table tr[data-pid="' +
							response.item_id +
							'"]'
					).remove();
				}
...
function create_new_contact(form) {
	jQuery.ajax({
		...
		success: function (response) {
			if (!response.error) {
				jQuery("#create-new-contact")
					.find('button[type="submit"]')
					.prop("disabled", false);
				jQuery("#create-new-contact")
					.find(".ajax-response")
					.html(
						'<span class="success">' + response.message + "</span>"
					);
				...
				jQuery("#create-new-contact")
					.find(".ajax-response")
					.html(
						'<span class="error">שגיאה בשליחת טופס. יש לסנות עוד פעם.</span>'
					);
			}
		},
	});
	...
	jQuery.ajax({
		...
		success: function (response) {
			...
			if (!response.error) {
				...
				jQuery(".reg-form .ajax-response").html(
...
function submit_step1_form(form) {
	jQuery.ajax({
		...
		success: function (response) {
			...
			if (response.error) {
				jQuery("#login-screen-wrapper .ajax-response").html(
					'<span class="error">' + response.message + "</span>"
				);
			} else {
			...
			if (response.html) {
				...
				jQuery(
					'<a href="' +
						...
						'" class="quick-go-to-product">לעמוד המוצר</a>'
				).insertAfter(".quick-view-product-inner .variations_form");
				jQuery(".quick-view-popup-holder").addClass("active");

				var variations = JSON.parse(
					jQuery(
						".quick-view-product-inner div.product form.cart"
				...
				if (response.product_share) {
					jQuery(
						".quick-view-product-inner .user-favorites-product"
					).append(response.product_share);
				}
			}
