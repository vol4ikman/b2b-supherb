...
jQuery(document).on('click', '.three-dots', function(e){
	e.preventDefault();
	jQuery(this).parents('.action-buttons-trigger').find('.actions-popup').toggleClass('active');
});
...
jQuery( ".single_variation_wrap" ).on( "show_variation", function ( event, variation ) {
	if( jQuery('body').hasClass('post-type-archive-product')){
        ...
        var variation_price_html = jQuery(event.currentTarget).find('.woocommerce-variation-price').html();
        jQuery(event.currentTarget).parents('.product-block-item').find('.product-price-wrapper').html(variation_price_html);
        jQuery(event.currentTarget).parents('.product-block-item').find('.product-price-wrapper').fadeTo(100, 1);

        // update variation id on add to favorite product button
        if( variation.variation_id ) {
            jQuery(event.currentTarget).parents('.product-block-item').find('.add-to-my-products').attr('data-item-id', variation.variation_id );
        }
    }
	...
	if( jQuery('body').hasClass('single-product')){
		...
		console.log(variation);
		var attribute_name = jQuery('table.variations select').attr('data-attribute_name');
	...
	if( jQuery('body').hasClass('quick-view-opened') ) {
		var attribute_name = jQuery('.quick-view-product-inner table.variations select').attr('data-attribute_name');
		var price_html = variation.price_html;
...
jQuery(document).on('change','.quick-view-product-inner .variations_form table.variations select',function(){
	console.log('quick-view variation changed');
	var variations = JSON.parse( jQuery(this).parents('form.cart').attr('data-product_variations') );
...
jQuery(document).on('click', '.quantity-trigger', function(e){
    ...
    var wc_quantity = jQuery(this).parents('.quantity').find('input.input-text.qty');
    var b2b_quantity = jQuery(this).parents('.b2b-quantity-inner').find('input[name="b2b-quantity"]');
	...
	if( jQuery('input[name="b2b-coupon-code"]').val() ) {
		...
		setTimeout( function(){
			if( jQuery('.woocommerce-error li').length ) {
				jQuery('.checkout-coupon-message').html( '<span class="message">'+jQuery('.woocommerce-error li').html()+'</span>' );
			}
		}, 800);
	} else {
...
jQuery(document).ready(function(){
	...
	if (jQuery(window).width() < 961) {
		jQuery('.MyAccount-mobile-navigation select').on('change', function (e) {
	...
	jQuery('.minicart-close button').on('click', function(e){
		e.preventDefault();
        ...
        jQuery(this).on('change', function(){
            ...
            if( ! validatePhoneNumber(input_val) ) {
                ...
                if( ! jQuery(this).parents('label .error-desc').length ){
                    jQuery(this).parents('label').append('<span class="error-desc">'+ jQuery(this).attr('placeholder') +' לא תקין</span>');
                }
            } else {
...
function googleTranslateElementInit() {
	...
	jQuery('.language-selector > div').on('click', function(e){
		...
		jQuery('.language-switcher-inner .current-lang').html( chosen_lang.toUpperCase() );
	});
}
