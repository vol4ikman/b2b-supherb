 ...
 */
function b2b_get_user_obligo( $user_id ) {
	$api_settings = get_field( 'user_api_response_data', 'user_' . $user_id );
	if ( $api_settings ) {
		$api_settings = json_decode( $api_settings, true );
		return $api_settings['MAX_OBLIGO'];
	}
	return 0;
}
